...
public class Cliente {
    ...
    private void actualizarTablero(String estado) {
        ...
        for (int i = 0; i < estado.length(); i++) {
            ...
            if (ficha == 'X') {
                // j1 usa cara.png
                ImageView[] imagenes = vista.getImagenes();
                Button[] botones = vista.getBotones();
                imagenes[i].setImage(new Image(getClass().getResourceAsStream("/cara.png")));
            } else if (ficha == 'O') {
                // j2 usa cruz.png
                ImageView[] imagenes = vista.getImagenes();
                imagenes[i].setImage(new Image(getClass().getResourceAsStream("/cruz.png")));
            }
            Button[] botones = vista.getBotones();
